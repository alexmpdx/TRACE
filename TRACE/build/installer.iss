...
; Mutually exclusive icon-variant choice, nested under desktopicon
; so the radio buttons only appear when the user actually wants a
; shortcut. ``exclusive`` makes them a single-choice group; "dark"
; comes first so it's the default selection (matches the white-on-dark
; logo most users want behind their TRACE icon).
Name: "desktopicon\dark"; Description: "Use dark icon"; \
  GroupDescription: "{cm:AdditionalIcons}"; Flags: exclusive
Name: "desktopicon\light"; Description: "Use light icon"; \
  GroupDescription: "{cm:AdditionalIcons}"; Flags: exclusive unchecked
...
; Ship both shortcut-icon variants next to TRACE.exe so the [Icons]
; section can point IconFilename: at whichever the user picked. These
; are regenerated by TRACE/build/make_installer_icon.py.
Source: "trace_icon_light.ico"; DestDir: "{app}"; Flags: ignoreversion
Source: "trace_icon_dark.ico"; DestDir: "{app}"; Flags: ignoreversion
...
;
; Two entries — one per icon variant. The user picks via the exclusive
; ``desktopicon\dark`` / ``desktopicon\light`` sub-tasks; whichever they
; selected is the one that fires here. IconFilename overrides the
; EXE's embedded icon for the shortcut only — the EXE itself still
; shows its own embedded icon in Explorer / Add-Remove Programs.
Name: "{autodesktop}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; \
  IconFilename: "{app}\trace_icon_dark.ico"; Tasks: desktopicon\dark
Name: "{autodesktop}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; \
  IconFilename: "{app}\trace_icon_light.ico"; Tasks: desktopicon\light
